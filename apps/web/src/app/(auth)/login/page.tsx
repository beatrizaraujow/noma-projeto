...
const getAuthErrorMessage = (errorCode: string | null) => {
  ...
  return AUTH_ERROR_MESSAGES[errorCode] || 'Falha ao autenticar com Google.';
};
...
export default function LoginPage() {
  ...
  useEffect(() => {
    const urlError = getAuthErrorMessage(authErrorFromUrl);
    if (urlError) {
      setError(urlError);
    }
  }, [authErrorFromUrl]);
  ...
  const handleGoogleLogin = async () => {
    if (!googleEnabled) {
      setError('Login com Google indisponivel neste ambiente.');
      return;
    }
    ...
    try {
      const origin = buildSignupOriginFromSearchParams(searchParams, 'login_google');
      ...
      <div className="relative z-10 flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-md">
          ...
          <p className="mt-6 text-center text-white">
            ...
            <Link href="/signup" className="font-semibold underline hover:text-orange-200">
              Criar uma
            </Link>
          </p>
        </div>
