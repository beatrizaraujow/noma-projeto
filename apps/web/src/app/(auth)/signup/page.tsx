...
import {
  buildSignupOriginFromSearchParams,
  persistGoogleSignupOrigin,
...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const origin = buildSignupOriginFromSearchParams(searchParams, 'signup_email');
      ...
      if (!signInResult?.ok) {
        throw new Error('Conta criada, mas nao foi possivel iniciar sessao automaticamente');
      }
  ...
  const handleGoogleSignup = async () => {
    if (!googleEnabled) {
      setError('Cadastro com Google indisponivel neste ambiente.');
      return;
    }
    ...
    try {
      const origin = buildSignupOriginFromSearchParams(searchParams, 'signup_google');
      await persistGoogleSignupOrigin(origin);
          ...
          <div className="bg-white rounded-2xl shadow-2xl p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-gray-500">Use ao menos 1 letra maiuscula, 1 minuscula e 1 numero</p>
              </div>
